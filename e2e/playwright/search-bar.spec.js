...
describe("The search bar component", () => {
  ...
  describe("basic component rendering", () => {
    test("it can be typed on", async ({ page }) => {
      ...
      let searchField = await page.getByRole('searchbox', { name: /search pokémon.../i })
      await searchField.fill("char")

      // Confirm there it has been typed on
      searchField = await page.getByRole('searchbox', { name: /search pokémon.../i })
      await expect(searchField).toHaveValue("char")
    })

    test("should display a proper message when no Pokémon is found", async ({ page }) => {
      // Target the search input field and fill it
      const searchField = await page.getByRole('searchbox', { name: /search pokémon.../i })
  ...
  describe("filtering the main page Pokémon list", () => {
    ...
    test("clearing the search term should display all available Pokémon again", async ({ page }) => {
      // Target the search input field and fill it
      let searchField = await page.getByRole('searchbox', { name: /search pokémon.../i })
      await searchField.fill("char")
  ...
  describe("testing how the search term affects the Pokémon info page", () => {
    test("the previous button should not be displayed on the first item of the filtered list", async ({ page }) => {
      // Target the search input field and fill it
      const searchField = await page.getByRole('searchbox', { name: /search pokémon.../i })
      await searchField.fill("char")
    ...
    test("the next button should not be displayed on the last item of the filtered list", async ({ page }) => {
      // Target the search input field and fill it
      const searchField = await page.getByRole('searchbox', { name: /search pokémon.../i })
      await searchField.fill("char")
